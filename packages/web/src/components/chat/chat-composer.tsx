...
export function ChatComposer({
  ...
  useEffect(() => {
    ...
    const nextHeight = Math.min(Math.max(element.scrollHeight, MIN_HEIGHT), MAX_HEIGHT);
  ...
  return (
    <div className={className}>
      ...
      <div
        className="isolate flex items-end gap-2 overflow-hidden rounded-[20px] px-2.5 py-2 shadow-[0_16px_36px_rgba(0,0,0,0.26)]"
        style={{
      ...
      >
        <textarea
          ...
          ref={textareaRef}
          className="chat-composer-textarea auto-grow-textarea w-full flex-1 bg-transparent px-1.5 py-1 text-[0.95rem] leading-6 text-white/92 placeholder:text-white/25 focus:outline-none"
          disabled={disabled}
      ...
      {errorMessage ? (
        <div className="px-1 pt-2">
          <p className="text-xs text-red-400/85">{errorMessage}</p>
        </div>
      ) : null}
    </div>
  );
