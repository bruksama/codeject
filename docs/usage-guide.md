...
## Cài global stop hooks

1. Chạy `npm run codeject -- install`.
2. Dùng `npm run codeject -- status` để kiểm tra wrapper/config đều healthy.
3. Nếu drift sau khi user sửa file config hoặc xóa wrapper, chạy `npm run codeject -- repair`.
4. Nếu muốn gỡ hoàn toàn, chạy `npm run codeject -- uninstall`.

Lưu ý hiện tại `install` chỉ hỗ trợ component `hooks`. `status` sẽ báo `healthy`, `drifted`, hoặc `not installed`; `repair` chỉ có tác dụng sau khi đã `install`.

> **Warning:** `uninstall` sẽ xóa luôn `~/.codeject`, nên config và session local của Codeject cũng bị xoá.
